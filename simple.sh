#!/bin/bash
# Builds llvm;clang;compiler-rt in src/build-${id} with
...
cmd="set -x ; \
  cmake ../llvm -G Ninja -DLLVM_ENABLE_PROJECTS=\"clang;compiler-rt\" -DCMAKE_INSTALL_PREFIX=${install_dir} -DCMAKE_BUILD_TYPE=Release -DLLVM_USE_LINKER=gold && \
  ninja -j${jobcnt} -v && \
  ninja -j${jobcnt} -v check-all"

# In a subshell time the evaluated command and then log the output
